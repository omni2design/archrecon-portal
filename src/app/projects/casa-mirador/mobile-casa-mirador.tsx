  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div
        aria-modal="true"
        role="dialog"
        className="relative w-full max-w-[420px] overflow-hidden rounded-[16px] border border-[#e5e5e5] bg-white shadow-[0_18px_50px_rgba(0,0,0,0.18)]"
      >
        <div className="flex items-start justify-between gap-4 border-b border-[#e5e5e5] px-6 py-4">
          <div>
            <p className="text-[16px] font-semibold leading-6 text-[#00162d]">Package Ready</p>
            <p className="mt-0.5 text-[12px] leading-4 text-[#6f6f6f]">
        ...
        <div className="px-6 py-5">
          <div className="flex flex-col gap-3">
            <button
              className="relative inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--ar-color-semantic-button-primary)] px-8 text-[var(--ar-color-semantic-button-primary-text)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--ar-color-primary-600)] disabled:opacity-80"
              onClick={startDownload}
              type="button"
              disabled={isPreparing}
            >
              <span className="text-[16px] font-medium leading-[19.2px]">
                {isPreparing ? "Preparing…" : "Download ZIP"}
              </span>
              <span className="absolute inset-0 rounded-full border border-[color:var(--ar-color-semantic-button-primary-boarder)]" />
              <DownloadIcon className="relative" />
            </button>

            <button
              className="flex h-[46px] w-full items-center justify-center gap-2 rounded-[48px] border border-[#e5e5e5] bg-white px-[17px] py-[13px] text-[14px] font-medium leading-5 text-[#00162d] transition hover:bg-[var(--ar-color-neutral-50)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--ar-color-primary-600)]"
              onClick={() => {
                onViewFiles();
                onClose();
              }}
            ...
            >
              View Files
              <ChevronRightIcon className="text-[#6f6f6f]" />
            </button>

            <p className="pt-1 text-center text-[12px] leading-4 text-[#6f6f6f]">
...
function MobileActivityIcon({ kind }: { kind: ActivityKind }) {
  const cls = "text-[var(--ar-color-primary-800)]";

  if (kind === "Uploads") {
    return (
      <svg aria-hidden className={cls} width="14" height="14" viewBox="0 0 24 24" fill="none">
        <path
...
function ActivityCard({ event }: { event: ActivityEvent }) {
  return (
    <div className="w-full rounded-[12px] border border-[#e5e5e5] bg-white p-4">
      <div className="flex items-start gap-4">
        <div className="flex size-8 shrink-0 items-center justify-center rounded-[6.667px] bg-[#f3f3f5] text-[var(--ar-color-primary-800)]">
          <MobileActivityIcon kind={event.kind} />
        </div>

        <div className="flex min-w-0 flex-1 items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[14px] font-medium leading-5 text-[#00162d]">{event.title}</p>
            <p className="text-[12px] leading-4 text-[#6f6f6f]">{event.description}</p>
          </div>

          <div className="flex shrink-0 flex-col items-center gap-1">
            <ActivityTag kind={event.kind} />
            <p className="text-[12px] leading-4 text-[#6f6f6f]">{event.timestampLabel}</p>
          </div>
        </div>
      </div>
    </div>
...
function MobileCasaMiradorContent() {
  ...
  const frontExterior = useMemo(() => assets.find((a) => a.id === "front-exterior-render") ?? assets[0], [assets]);

  const onViewFiles = () => {
    setActiveTab("drafting-design");
    setActiveFilter("All");
    document.getElementById("casa-mirador-mobile-tabs")?.scrollIntoView({ behavior: "smooth" });
  };
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-[#e5e5e5] bg-white px-6 pb-0 pt-6">
        ...
        <div
          className="mt-4 flex items-center justify-center gap-8 border-b border-[#e5e5e5]"
          id="casa-mirador-mobile-tabs"
        >
          <ProjectTab
      ...
      <div className="px-6 pt-6" style={{ paddingBottom: MOBILE_CASA_MIRADOR_BOTTOM_PADDING }}>
        {activeTab === "drafting-design" ? (
          <div className="flex flex-col gap-4">
            <div className="-mx-6 flex gap-2 overflow-x-auto overflow-y-visible px-6 pb-3 pt-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              className="pt-1 text-center text-[14px] font-medium leading-5 text-[#6f6f6f] hover:text-[#00162d]"
              onClick={() => {}}
              style={{ fontFamily: "var(--ar-font-family-body)" }}
              type="button"
            >
          ...
          <div className="flex flex-col gap-16">
            ...
            <div className="overflow-hidden rounded-[12px] border border-[#e5e5e5] bg-white">
              <div className="relative h-[248px] w-full overflow-hidden bg-[#f3f3f5]">
                {frontExterior ? (
                  <Image
                ...
                <div className="absolute inset-x-0 bottom-0 h-[253px] bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
                <div className="absolute bottom-5 left-6 right-6 flex flex-wrap items-end gap-2">
                  <span className="inline-flex items-center justify-center overflow-hidden rounded-full border border-[var(--ar-color-state-success-bg)] bg-[var(--ar-color-state-success-boarder)] px-3 py-1 text-[14px] font-medium leading-5 text-[var(--ar-color-state-success-text)]">
                    Completed
                  </span>
                  <span className="inline-flex items-center justify-center overflow-hidden rounded-full bg-white/75 px-3 py-1 text-[14px] font-medium leading-5 text-black">
                    Passive Tropical Residence
                  </span>
                  <span className="inline-flex items-center justify-center overflow-hidden rounded-full bg-white/75 px-3 py-1 text-[14px] font-medium leading-5 text-black">
                    Chakabamba, Peru
                  </span>
              ...
              <div className="flex flex-col gap-3 p-6">
                <p className="text-[18px] font-semibold leading-[22px] text-[#00162d]">Project Description</p>
                <p className="text-[14px] leading-5 text-[#6f6f6f]">
                  A comprehensive drafting and design package for a contemporary tropical residence in Chakabamba, Peru.
                  The project includes a full design set documentation, photorealistic renderings, and preliminary
                  construction-ready drawing sets for a passive design residence featuring indoor-outdoor living spaces,
                  natural ventilation, and integration with the surrounding landscape.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={c.label} className="flex flex-col gap-4 rounded-[12px] border border-[#e5e5e5] bg-white p-4">
                  <div className="flex size-12 items-center justify-center rounded-[10px] bg-[#f3f3f5] text-[#00162d]">
                    {c.icon}
                  </div>
                  <div>
                    <p className="text-[12px] leading-4 text-[#6f6f6f]">{c.label}</p>
                    <p className="text-[14px] font-medium leading-5 text-[#00162d]">{c.value}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Project highlights */}
            <div className="flex flex-col gap-4">
              <h2 className="font-[family-name:var(--ar-font-family-heading)] text-2xl font-medium leading-8 text-[#00162d]">
                Project Highlights
              </h2>
              <div
                ...
                style={{
                  backgroundImage:
                    "linear-gradient(150.43586188746679deg, rgba(241, 114, 171, 0.1) 0%, rgba(127, 195, 232, 0.1) 50%, rgba(241, 114, 171, 0.1) 100%)",
                }}
              >
                <div className="flex flex-col gap-4">
                  {[
                    ...
                    <div key={h.title} className="flex items-center gap-4">
                      <div className="size-8">
                        ...
                        >
                          <path
                            ...
                            fill="url(#paint0_linear_2482_3777_mobile)"
                          />
                          <path
                          ...
                          <defs>
                            <linearGradient
                              id="paint0_linear_2482_3777_mobile"
                              x1="0"
                              y1="16"
                              x2="32"
                              y2="16"
                      ...
                      <div>
                        <p className="text-[14px] font-medium leading-5 text-[#00162d]">{h.title}</p>
                        <p className="text-[12px] leading-4 text-[#6f6f6f]">{h.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-[12px] border border-[#e5e5e5] bg-white p-6">
                <div className="flex flex-col gap-4">
                  <div>
                    <p className="text-[12px] leading-4 text-[#6f6f6f]">Primary Contact</p>
                    <p className="text-[14px] font-medium leading-5 text-[#00162d]">ArchRecon Studio</p>
                  </div>
                  <div>
                    <p className="text-[12px] leading-4 text-[#6f6f6f]">Team Members</p>
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-[14px] font-medium leading-5 text-[#00162d]">3 members</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-[12px] leading-4 text-[#6f6f6f]">Location</p>
                    <div className="flex items-center gap-2">
                      <svg
                        aria-hidden="true"
                      ...
                      <p className="text-[14px] font-medium leading-5 text-[#00162d]">Chakabamba, Peru</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-4">
                {[
                  ...
                  <div key={card.title} className="rounded-[12px] border border-[#e5e5e5] bg-white p-6">
                    <div className="flex items-start justify-between">
                      <div className="flex size-12 items-center justify-center rounded-[10px] bg-[#f3f3f5]">
                        {card.icon}
                      </div>
                      <p className="text-[12px] leading-4 text-[#6f6f6f]">{card.count}</p>
                    </div>
                    <div className="mt-4">
                      <p className="text-[14px] font-medium leading-5 text-[#00162d]">{card.title}</p>
                      <p className="mt-0.5 max-w-[214px] text-[12px] leading-4 text-[#6f6f6f]">
                        {card.subtitle}
                      </p>
                      <ul className="mt-2 list-disc text-[12px] leading-4 text-[#6f6f6f]">
                        {card.bullets.map((b) => (
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    document
                      .getElementById("casa-mirador-mobile-tabs")
                ...
                <div className="flex flex-col gap-3">
                  {[
                    ...
                    <div
                      key={row.id}
                      className={
                        idx === arr.length - 1 ? "flex gap-3" : "flex gap-3 border-b border-[#e5e5e5] pb-[13px]"
                      }
                    >
                      ...
                      <div className="flex flex-1 flex-col gap-1">
                        <p className="text-[14px] font-medium leading-5 text-[#00162d]">{row.title}</p>
                        <p className="text-[12px] leading-4 text-[#6f6f6f]">{row.time}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-[12px] border border-[#e5e5e5] bg-white p-6">
                <div className="flex flex-col gap-2">
                  <button
                    className="flex h-[46px] w-full items-center justify-center gap-2 rounded-[48px] border border-[#e5e5e5] bg-white px-[17px] py-[13px] text-[14px] font-medium leading-5 text-[#00162d] transition hover:bg-[var(--ar-color-neutral-50)]"
                    onClick={onViewFiles}
                    type="button"
                  >
                    Browse Files
                    <svg aria-hidden="true" width="16" height="16" viewBox="0 0 16 16" fill="none" className="text-[#6f6f6f]">
      ...
      {/* Fixed mobile CTA strip */}
      <div className="fixed bottom-[calc(var(--ar-bottom-offset,0px)+var(--ar-mobile-bottom-nav-height,0px))] left-0 right-0 z-40 border-t border-[#e5e5e5] bg-white/90 px-6 py-4 backdrop-blur">
        <button
          className="relative inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--ar-color-semantic-button-primary)] px-8 text-[var(--ar-color-semantic-button-primary-text)]"
