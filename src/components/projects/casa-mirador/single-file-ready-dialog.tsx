...
function safeFileBaseName(title: string) {
  return title.replace(/[^a-z0-9]+/gi, "_").replace(/^_|_$/g, "") || "Casa_Mirador_File";
}

export function CasaMiradorSingleFileReadyDialog({
  isOpen,
  onClose,
  fileTitle,
  onViewAllFiles,
  ...
  const startDownload = async () => {
    ...
    const content = [
      "ArchRecon — Single file export (Demo)",
      "",
      `File: ${fileTitle}`,
      "This is a placeholder for the file you are viewing.",
      "In production, this would be the real project deliverable.",
      "",
    ...
    a.href = url;
    a.download = `${safeFileBaseName(fileTitle)}.txt`;
        ...
        <div className="flex items-start justify-between gap-4 border-b border-[#e5e5e5] px-6 py-4">
          <div>
            ...
            <p
              ...
              style={{ fontFamily: "var(--ar-font-family-body)" }}
            >
              Download only this file — not the full project package.
            </p>
          </div>
